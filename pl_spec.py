...
def pl_spec_lf(xdim, ydim, dx, dy, foldername, current_user, center=(0, 0),
               ...
               scan_type='coarse', data_folder='data'):
    """
    Same as pl_spec_manual but uses the LightField Python backend (lf_spec)
    instead of the MATLAB engine.
    ...
    Args:
        ...
        dy (float): Step size in the y direction.
        foldername (str): Base name of the folder where data will be saved.
        current_user (str): Telegram identifier for out of focus alerts.
        center (tuple): (x, y) coordinates for the center of the scan. Default is (0, 0).
        grating (int): Spectrometer grating option. Allowed values are 150 or 600.
        exposure_time (float): Spectrometer exposure time in seconds.
        center_wavelength (int): Spectrometer center wavelength in nanometers.
        classification_threshold (float): Minimum fraction of the laser peak that emitter peak should be.
        scan_type (str): Subfolder category for the scan (e.g., 'coarse', 'fine').
        data_folder (str): The root directory for all saved data.
    """
